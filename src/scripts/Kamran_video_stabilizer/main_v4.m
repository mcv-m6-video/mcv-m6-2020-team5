...
finalConvexRotate = zeros(size(finalConvex));
...
finalConvexRotate(:,:,1) = finalConvex(:,:,1);
    ...
    for i = 1:finalHeight
        for j = 1:finalWidth
            ...
            if (((1-newHeightOrigin) <= newXPosition) && (newXPosition <= (height-newHeightOrigin)) && ((1-newWidthOrigin) <= newYPosition) && (newYPosition <= (width-newWidthOrigin)))
                finalConvexRotate(i,j,k+1) = finalConvex(startPoint(:,1)+newXPosition+newHeightOrigin , startPoint(:,2)+newYPosition+newWidthOrigin , k+1);
            end
        end
    end
...
finalResult1 = [doubleGrayVideoResize finalConvexRotate];
