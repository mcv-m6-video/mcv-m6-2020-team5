...
corners = zeros(1,2,4);
cornersHeightInOrder = zeros(1,1,4);
cornersWidthInOrder = zeros(1,1,4);
maxHeight = 1;
minHeight = finalHeight;
maxWidth = 1;
minWidth = finalWidth;
finalMaxHeight = finalHeight;
finalMinHeight = 0;
finalMaxWidth = finalWidth;
finalMinWidth = 0;
...
for k = 1:num
    ...
    totalAlpha = totalAlpha + alphaShakeMotion(k);
    ...
    cornersHeightInOrder = sort(corners(1,1,:));
    cornersWidthInOrder = sort(corners(1,2,:));
    finalMaxHeight = min(finalMaxHeight , cornersHeightInOrder(1,1,3));
    finalMinHeight = max(finalMinHeight , cornersHeightInOrder(1,1,2));
    finalMaxWidth = min(finalMaxWidth , cornersWidthInOrder(1,1,3));
    finalMinWidth = max(finalMinWidth , cornersWidthInOrder(1,1,2));
...
% 
% save('finalConvex.mat' , 'finalConvex' , '-v7.3');
% finalConvex = uint8(finalConvex);
% writerObj1 = VideoWriter('finalConvex.avi' , 'Uncompressed AVI');
% open(writerObj1);
% for i = 1:num+1
%     writeVideo(writerObj1 , finalConvex(:,:,i));
% end
% close(writerObj1);
% 
% save('doubleGrayVideoResize.mat' , 'doubleGrayVideoResize' , '-v7.3');
% doubleGrayVideoResize = uint8(doubleGrayVideoResize);
% writerObj1 = VideoWriter('doubleGrayVideoResize.avi' , 'Uncompressed AVI');
% open(writerObj1);
% for i = 1:num+1
%     writeVideo(writerObj1 , doubleGrayVideoResize(:,:,i));
% end
% close(writerObj1);
% 
...
% padHeight = 10;
% padWidth = 10;
% finalResult2 = zeros(padHeight+(finalMaxHeight-(finalMinHeight-1))+padHeight , padWidth+(finalMaxWidth-(finalMinWidth-1))+padWidth+(finalMaxWidth-(finalMinWidth-1))+padWidth , num+1);
% finalResult2(padHeight+1:padHeight+(finalMaxHeight-(finalMinHeight-1)) , padWidth+1:padWidth+(finalMaxWidth-(finalMinWidth-1)) , :) = doubleGrayVideoResize(finalMinHeight:finalMaxHeight , finalMinWidth:finalMaxWidth , 1:num+1);
% finalResult2(padHeight+1:padHeight+(finalMaxHeight-(finalMinHeight-1)) , padWidth+(finalMaxWidth-(finalMinWidth-1))+padWidth+1:padWidth+(finalMaxWidth-(finalMinWidth-1))+padWidth+(finalMaxWidth-(finalMinWidth-1)) , :) = finalConvex(finalMinHeight:finalMaxHeight , finalMinWidth:finalMaxWidth , 1:num+1);
% implay(uint8(finalResult2));
% save('finalResult2.mat' , 'finalResult2' , '-v7.3');
% finalResult2 = uint8(finalResult2);
% writerObj1 = VideoWriter('finalResult2.avi' , 'Uncompressed AVI');
% open(writerObj1);
% for i = 1:num+1
%     writeVideo(writerObj1 , finalResult2(:,:,i));
% end
% close(writerObj1);
% clear finalResult2;
% clear finalConvex;
