...
for k = 1:num
    ...
    inliers1 = zeros(2,numberOfInliers);
    inliers2 = zeros(2,numberOfInliers);
    ...
    x(k) = round(x(k)/numberOfInliers);
    y(k) = round(y(k)/numberOfInliers);
    
end

xSmoothMotion = smooth(x);
ySmoothMotion = smooth(y);
for k = 1:num
    ...
    for i = 1:height
        for j = 1:width
            if ((u1(i,j,k) ~= 0) || (v1(i,j,k) ~= 0))
                l = l + 1;
                m1(: , l) = [i , j];
                m2(: , l) = [i+v1(i,j,k)-y(k) , j+u1(i,j,k)-x(k)];
            end
        end
    end
